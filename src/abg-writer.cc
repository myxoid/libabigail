...
public:
  ...
  record_type_as_emitted(const type_base* t)
  {
    if (auto d = const_cast<class_decl*>(dynamic_cast<const class_decl*>(t))) {
      bool debug = d->get_name() == "sockaddr";
      if (debug) {
        ...
        std::cerr << "RECORD_TYPE_AS_EMITTED:"
                  ...
                  << " decl_only_is_emitted=" << decl_only_type_is_emitted(t)
                  ...
                  << " look_through_decl_only_is_emitted=" << decl_only_type_is_emitted(look_through)
                  << '\n';
      }
    }
  ...
  record_decl_only_type_as_emitted(type_base* t)
  {
    if (auto d = const_cast<class_decl*>(dynamic_cast<const class_decl*>(t))) {
      ...
      if (debug) {
        auto look_through = is_class_type(look_through_decl_only_class(d));
        std::cerr << "RECORD_DECL_ONLY_TYPE_AS_EMITTED:"
                  << " class_decl=" << d
                  << " type_base=" << t
                  << " canonical_type=" << t->get_naked_canonical_type()
                  << " is_declaration_only=" << d->get_is_declaration_only()
		     ...
		     unsigned indent)
{
  ...
  o << "<underlying-type type-id='"
    ...
    << "'/>\n";
  // Don't record the underlying type as referenced or we'll end up a
  // duplicate in the XML. I don't know why.
		    ...
		    write_context& ctxt, unsigned indent)
{
  ...
  if (method_type_sptr method_type = is_method_type(fn_type))
	...
	<< "'";
      ctxt.record_type_as_referenced(method_type->get_class_type());
		 ...
		 unsigned		indent)
{
  ...
  if (debug) {
    ...
    std::cerr << "WRITE_CLASS_DECL:"
              ...
              << " decl_only_is_emitted=" << ctxt.decl_only_type_is_emitted(d)
              ...
              << " look_through_decl_only_is_emitted=" << ctxt.decl_only_type_is_emitted(look_through)
              << '\n';
  }
