...
type_suppression::offset::integer_offset::eval(
  class_decl_sptr, ssize_t& value) const
{
  value = priv_->value_;
  return true;
}
...
type_suppression::offset::fn_call_expr_offset::eval(
  class_decl_sptr context, ssize_t&value) const
{
		      ...
		      suppression_sptr& suppr)
{
  ...
  static const std::map<std::string, property_info<S>> info = {
    ...
    { "file_name_regexp",       { 1, 0, &S::set_file_name_regex          } },
    { "file_name_not_regexp",   { 1, 0, &S::set_file_name_not_regex      } },
    { "soname_regexp",          { 1, 0, &S::set_soname_regex             } },
    { "soname_not_regexp",      { 1, 0, &S::set_soname_not_regex         } },
    { "name_regexp",            { 1, 0, &S::set_type_name_regex          } },
    { "name_not_regexp",        { 1, 0, &S::set_type_name_not_regex      } },
    { "name",                   { 1, 0, &S::set_type_name                } },
    { "source_location_not_in", { 1, 0, &S::set_source_locations_to_keep } },
    { "source_location_not_regexp",
      { 1, 0, &S::set_source_location_to_keep_regex } },
					  ...
					  const diff_context_sptr ctxt) const
{
  ...
  if (!get_return_type_name().empty())
    if (fn_return_type_name != get_return_type_name())
      return false;
  if (const regex_t_sptr& regex = get_return_type_regex())
	   ...
	   ++p)
	{
	  ...
	  if (!tn.empty())
	    if (tn != fn_parm_type_qualified_name)
	      return false;
	  if (const regex_t_sptr& regex = (*p)->get_parameter_type_name_regex())
						 ...
						 const diff_context_sptr ctxt)
{
  ...
  if (const regex_t_sptr& regex = get_symbol_name_regex())
    if (!regex::match(regex, sym_name))
      return false;
  bool no_symbol_name = get_symbol_name().empty() && !get_symbol_name_regex();;
  ...
  if (const regex_t_sptr& regex = get_symbol_version_regex())
    if (!regex::match(regex, sym_version))
      return false;
  bool no_symbol_version =
    get_symbol_version().empty() && !get_symbol_version_regex();
				  ...
				  const string& fn_name)
{
  ...
  if (const regex_t_sptr& regexp = s.get_name_regex())
    if (!regex::match(regexp, fn_name))
      return false;
  if (const regex_t_sptr& regexp = s.get_name_not_regex())
    if (regex::match(regexp, fn_name))
      return false;

  // TODO: Why?
  if (s.get_name().empty() && !s.get_name_regex() && !s.get_name_not_regex())
    return false;
				      ...
				      const string& fn_linkage_name)
{
  ...
  if (const regex_t_sptr& regexp = s.get_symbol_name_regex())
    if (!regex::match(regexp, fn_linkage_name))
      return false;
  if (const regex_t_sptr& regexp = s.get_symbol_name_not_regex())
    if (regex::match(regexp, fn_linkage_name))
      return false;

  // TODO: Why?
  if (s.get_symbol_name().empty()
      && !s.get_symbol_name_regex()
      && !s.get_symbol_name_not_regex())
    return false;
			  ...
			  suppression_sptr& suppr)
{
  ...
  static const std::map<std::string, property_info<S>> info = {
    ...
    { "soname_not_regexp",      { 1, 0, &S::set_soname_not_regex       } },
    { "name",                   { 1, 0, &S::set_name                   } },
    { "name_regexp",            { 1, 0, &S::set_name_regex             } },
    { "name_not_regexp",        { 1, 0, &S::set_name_not_regex         } },
    { "return_type_name",       { 1, 0, &S::set_return_type_name       } },
    { "return_type_regexp",     { 1, 0, &S::set_return_type_regex      } },
    { "symbol_name",            { 1, 0, &S::set_symbol_name            } },
    { "symbol_name_regexp",     { 1, 0, &S::set_symbol_name_regex      } },
    { "symbol_name_not_regexp", { 1, 0, &S::set_symbol_name_not_regex  } },
    { "symbol_version",         { 1, 0, &S::set_symbol_version         } },
    { "symbol_version_regexp",  { 1, 0, &S::set_symbol_version_regex   } },
    { "allow_other_aliases",    { 0, 0, &S::set_allow_other_aliases    } },
					  ...
					  const diff_context_sptr ctxt) const
{
  ...
  // Check if the "name_regex" and "name_not_regex" properties match
						 ...
						 const diff_context_sptr ctxt) const
{
  ...
  if (const regex_t_sptr& regex = get_symbol_name_regex())
    if (!regex::match(regex, sym_name))
      return false;
  // TODO: What happened to get_symbol_name_not_regex?
  ...
  // TODO: Why?
  bool no_symbol_name =
    get_name().empty() && get_symbol_name().empty() && !get_symbol_name_regex();
  bool no_symbol_version =
    get_symbol_version().empty() && !get_symbol_version_regex();
  if (no_symbol_name && no_symbol_version)
    return false;
			  ...
			  suppression_sptr& suppr)
{
  ...
  static const std::map<std::string, property_info<S>> info = {
    ...
    { "type_name_regexp",       { 1, 0, &S::set_type_name_regex        } },
  };
...
file_suppression::suppresses_file(const string& file_path)
{
  ...
  // TODO: Why?
  bool has_regexp = get_file_name_regex() || get_file_name_not_regex();
  if (!has_regexp)
    return false;
